...
lblHomeDelivery.grid(row=4, column=0)
txtHomeDelivery = Entry(f1ab, font=('arial', 16, 'bold'), textvariable=CostofHomeDelivery, bd=10, insertwidth=2,
                        justify='left')
txtHomeDelivery.grid(row=4, column=1)
...
def CostOfOrder():
    ...
    ToC = "$ " + str(
        "%.2f" % ((CarpetPrice * 15.49) + (BlindsPrice * 7.49) + (FabricPrice * 5.50) + (DeliveryCost * 4.50)))
    SubTotal.set(ToC)

    Tax = "$ " + str(
        "%.2f" % (((CarpetPrice * 15.49) + (BlindsPrice * 7.49) + (FabricPrice * 5.50) + (DeliveryCost * 4.50)) * 0.2))
    PaidTax.set(Tax)
...
btnTotal = Button(f2ab, padx=16, pady=16, bd=8, fg='black', font=('arial', 16, 'bold'), width=15, text='Total Cost',
                  command=CostOfOrder).grid(row=0, column=0)
...
btnReset = Button(f2ab, padx=16, pady=16, bd=8, fg='black', font=('arial', 16, 'bold'), width=15, text='Reset',
                  command=Reset).grid(row=1, column=0)

btnExit = Button(f2ab, padx=16, pady=16, bd=8, fg='black', font=('arial', 16, 'bold'), width=15, text='Exit',
                 command=iExit).grid(row=1, column=1)
